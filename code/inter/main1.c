...
int main(){
	...
	fil=fopen("energy.dat","w");
	fiil=fopen("fidel.dat","w");
	for(int k=1;k<50;k++){	
	...
	for(int i=0;i<L;i++){
		fprintf(fi,"%i	%g\n",i,sqrt(pow(GSL_REAL(gsl_matrix_complex_get(evec,i,st)),2)+pow(GSL_IMAG(gsl_matrix_complex_get(evec,i,st)),2)));
	...
	complex double psi_l[2]={1./sqrt(2),-1./sqrt(2)*I}, psi_r[2]={1./sqrt(2),1./sqrt(2)*I};
	complex double l_edge[2]={GSL_REAL(gsl_matrix_complex_get(evec,L_1,0))+I*GSL_IMAG(gsl_matrix_complex_get(evec,L_1,0)),GSL_REAL(gsl_matrix_complex_get(evec,L_1+1,0))+I*GSL_IMAG(gsl_matrix_complex_get(evec,L_1+1,0))};
	complex double r_edge[2]={GSL_REAL(gsl_matrix_complex_get(evec,L_1+L_c-2,0))+I*GSL_IMAG(gsl_matrix_complex_get(evec,L_1+L_c-2,0)),GSL_REAL(gsl_matrix_complex_get(evec,L_1+L_c-1,0))+I*GSL_IMAG(gsl_matrix_complex_get(evec,L_1+L_c-1,0))};

	if(creal(conj(r_edge[0])*r_edge[0])<creal(conj(l_edge[0])*l_edge[0])){

	l_edge[0]/=(l_edge[0]*conj(l_edge[0])+l_edge[1]*conj(l_edge[1]));
	l_edge[0]/=(l_edge[0]*conj(l_edge[0])+l_edge[1]*conj(l_edge[1]));
	over=conj(l_edge[0])*psi_l[0]+conj(l_edge[1])*psi_l[1];
	}

	else{

	r_edge[1]/=(r_edge[0]*conj(r_edge[0])+r_edge[1]*conj(r_edge[1]));
	r_edge[1]/=(r_edge[0]*conj(r_edge[0])+r_edge[1]*conj(r_edge[1]));
	over=conj(r_edge[0])*psi_r[0]+conj(r_edge[1])*psi_r[1];
	}

	fprintf(fiil,"%g\n",creal(over*conj(over)));
