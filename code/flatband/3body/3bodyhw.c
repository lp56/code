...
void onsite(int i, gsl_matrix *M, int N, double U){
	...
	int *x=&a, *y=&b, *z=&c;
	findxyz(i,x,y,z,N);
	if (*x==*y && *y==*z) gsl_matrix_set(M,index(x,y,z,N),i,0.5*9*U+gsl_matrix_get(M,index(x,y,z,N),i));
	else if(*x==*y||*x==*z||*y==*z) gsl_matrix_set(M,index(x,y,z,N),i,0.5*5*U+gsl_matrix_get(M,index(x,y,z,N),i));
}
